# train.py – single‑node multi‑GPU with tqdm progress + W&B batch logging
...
def run_worker(rank: int, world: int, cfg: dict):
    ...
    #split_data = torch.load(cfg["split_file"], weights_only = False),
    ...
    global_step = 0
    log_every   = cfg.get("log_every", 50)           # batches between W&B logs
    ...
    for epoch in range(cfg["epochs"]):
        ...
        for feats, t, e in bar:
            ...
            if rank == 0:
                ...
                if cfg["wandb"]["mode"] != "disabled" and global_step % log_every == 0:
                    wandb.log({"train/loss": loss.item(), "step": global_step}, commit=False)
            global_step += 1
        ...
        if rank == 0:
            ...
            if cfg["wandb"]["mode"] != "disabled":
                wandb.log({"val/ci": ci, "val/auc": auc, "epoch": epoch})
            net.train()
